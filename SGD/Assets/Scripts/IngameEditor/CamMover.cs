...
namespace IngameEditor
{
    public class CamMover : MonoBehaviour
    {
        ...
        private float _initialSnapshot;
        
        private void Start()
        {
            target = GetComponentInChildren<Camera>();
            _initialSnapshot = Time.time;
            target.transform.localPosition = Vector3.back * initialZoomRange;
            LeanTween.moveLocalZ(target.gameObject, -defaultZoom, initialZoomTime).setEaseOutCubic();
        }
